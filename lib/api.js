...
export const createZeroTrustListsOneByOne = async (items) => {
  ...
  for (let i = 0, listNumber = 1; i < items.length; i += LIST_ITEM_SIZE) {
    const chunk = items
      ...
      .map((item) => ({ value: item }));
    const listName = ZCGPS List - Chunk ${listNumber}`;
 ...
 * @returns {Promise<Object>}
 */
export const upsertZeroTrustRule = async (wirefilterExpression, name = "ZGPS Filter Lists", filters = ["dns"]) => {
 ...
 * @returns {Promise<Object>}
 */
export const createZeroTrustRule = async (wirefilterExpression, name = "ZGPS Filter Lists", filters = ["dns"]) => {
  try {
    await requestGateway("/rules", {
      ...
      body: JSON.stringify({
        ...
        enabled: true,
        action: "block",
        rule_settings: { "block_page_enabled": BLOCK_PAGE_ENABLED, "block_reason": "Blocked by ZGPS, check your filter lists if this was a mistake." },
        filters,
 ...
 * @returns {Promise<Object>}
 */
export const updateZeroTrustRule = async (id, wirefilterExpression, name = "ZGPS Filter Lists", filters = ["dns"]) => {
  try {
