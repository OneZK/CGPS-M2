...
const wirefilterDNSExpression = lists.reduce((previous, current) => {
  if (!current.name.startsWith("ZGPS List")) return previous;
...
await upsertZeroTrustRule(wirefilterDNSExpression.slice(0, -4), "ZGPS Filter Lists", ["dns"]);
...
if (BLOCK_BASED_ON_SNI) {
  ...
  // .slice removes the trailing ' or '
  await upsertZeroTrustRule(wirefilterSNIExpression.slice(0, -4), "ZGPS Filter Lists - SNI Based Filtering", ["l4"]);
}
